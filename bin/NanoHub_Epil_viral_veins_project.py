...
def write_config_file_cb(b):
    path_to_share = os.path.join('~', '.local','share','NanoHub_Epil_viral_veins_project')
...
def get_config_files():
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/NanoHub_Epil_viral_veins_project")  # does Windows like this?
    else:
        # local cache
        try:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB NanoHub_Epil_viral_veins_project' -t 'Your Run completed.'&")
...
def run_sim_func(s):
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 NanoHub_Epil_viral_veins_project-r7 config.xml")   # "-r7" suffix??
        else:
...
tool_title = widgets.Label(r'\(\textbf{NanoHub_Epil_viral_veins_project}\)')
